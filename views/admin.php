...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
  ...
  // ── XỬ LÝ QUẢN LÝ PHÒNG (VÁ LỖI XÓA & BỔ SUNG SỬA TRẠNG THÁI) ──
  if ($hanhDong === 'themPhong') {
    $maPhong = trim($_POST['maPhong'] ?? '');
    $soPhong = trim($_POST['soPhong'] ?? '');
    $maKS = trim($_POST['maKS'] ?? '');
    $maLoai = trim($_POST['maLoai'] ?? '');
    $stmtC = $pdo->prepare("SELECT COUNT(*) FROM phong WHERE MaPhong=?");
    $stmtC->execute([$maPhong]);
    if ($stmtC->fetchColumn() > 0) {
      $thongBaoLoi = "Mã phòng đã tồn tại.";
    } else {
  ...
  // BỔ SUNG: Tính năng Sửa trạng thái phòng dành cho Admin quản trị
  if ($hanhDong === 'suaPhong') {
    $maP = $_POST['maPhong'];
    $trangThaiMoi = $_POST['trangThai'];
    $stmtU = $pdo->prepare("UPDATE phong SET TrangThai = ? WHERE MaPhong = ?");
    if ($stmtU->execute([$trangThaiMoi, $maP])) {
      $thongBaoThanhCong = "Cập nhật trạng thái phòng $maP thành công!";
    } else {
  ...
  if ($hanhDong === 'xoaPhong') {
    $maP = $_POST['maPhong'];
    $stmtC = $pdo->prepare("SELECT COUNT(*) FROM don_dat_phong WHERE MaPhong=? AND TrangThaiDon IN ('ChoXacNhan','DaXacNhan')");
    $stmtC->execute([$maP]);
    if ($stmtC->fetchColumn() > 0) {
      $thongBaoLoi = "Phòng đang có lịch đặt sắp diễn ra, nghiêm cấm xóa khỏi hệ thống.";
    } else {
      try {
        // VÁ LỖI: Bọc try-catch phòng ngừa xung đột khóa ngoại trên CSDL khi có hóa đơn cũ liên kết
        $pdo->prepare("DELETE FROM phong WHERE MaPhong=?")->execute([$maP]);
        $thongBaoThanhCong = "Xóa phòng thành công khỏi hệ thống.";
      } catch (PDOException $e) {
        $thongBaoLoi = "Không thể xóa phòng này vì đã có dữ liệu lịch sử hóa đơn lưu trú cũ ràng buộc.";
      }
    }
  }
  ...
  if ($hanhDong === 'capNhatDon') {
    ...
    if (in_array($trangThaiMoi, ['DaXacNhan', 'HoanTat', 'DaHuy'])) {
      ...
      if ($trangThaiMoi === 'DaHuy' || $trangThaiMoi === 'HoanTat') {
        $stmtP = $pdo->prepare("SELECT MaPhong FROM don_dat_phong WHERE MaDon = :id");
        $stmtP->execute([':id' => $maDon]);
  ...
  if ($hanhDong === 'xoaPhong') {
    ...
    if ($stmtC->fetchColumn() > 0) $thongBaoLoi = "Phòng đang có khách đặt, cấm xóa.";
      ...
      $pdo->prepare("DELETE FROM phong WHERE MaPhong=?")->execute([$maP]);
      $thongBaoThanhCong = "Xóa phòng xong.";
    }
  }
  ...
  if ($hanhDong === 'themTaiKhoan') {
    ...
    $em = trim($_POST['email'] ?? '');
    $dt = trim($_POST['sdt'] ?? '');
    $vt = $_POST['vaiTro'] ?? 'ThanhVien';
    $mk = $_POST['matKhau'] ?? '';
    $stmtC = $pdo->prepare("SELECT COUNT(*) FROM tai_khoan WHERE Email=? OR SDT=?");
    $stmtC->execute([$em, $dt]);
  ...
  if ($hanhDong === 'xoaTaiKhoan') {
    ...
    if ($id == $_SESSION['MaTK']) $thongBaoLoi = "Không thể tự xóa chính mình.";
      ...
      $pdo->prepare("DELETE FROM tai_khoan WHERE MaTK=?")->execute([$id]);
      $thongBaoThanhCong = "Xóa user xong.";
    }
  }
}
...
$dsKS = $pdo->query("SELECT * FROM khach_san")->fetchAll();
$dsLP = $pdo->query("SELECT * FROM loai_phong")->fetchAll();

$pageTitle = 'Luxury Hotel – Admin';
  ...
  <main class="flex-fill p-4 bg-light">
    ...
    <?php if ($trang === 'dashboard'): ?>
      ...
      <div class="row g-3 mb-4">
        <div class="col-md-4">
          <div class="card p-3 shadow-sm border-0"><small class="text-muted">TỔNG ĐƠN ĐẶT PHÒNG</small>
            <h3 class="fw-bold mt-1 text-navy"><?= count($dsDonAdmin) ?></h3>
          </div>
        </div>
        <div class="col-md-4">
          <div class="card p-3 shadow-sm border-0"><small class="text-muted">TỔNG SỐ PHÒNG</small>
            <h3 class="fw-bold mt-1 text-success"><?= count($dsPhongAdmin) ?></h3>
          </div>
        </div>
        <div class="col-md-4">
          <div class="card p-3 shadow-sm border-0"><small class="text-muted">TÀI KHOẢN KHÁCH</small>
            <h3 class="fw-bold mt-1 text-warning"><?= count($dsTaiKhoanAdmin) ?></h3>
          </div>
        </div>
      </div>
    ...
    <?php elseif ($trang === 'rooms'): ?>
      <div class="d-flex justify-content-between mb-3">
        <h3 class="font-playfair mb-0">Quản lý phòng</h3><button class="btn btn-gold btn-sm" data-bs-toggle="modal" data-bs-target="#mP">+ Thêm phòng</button>
      </div>
      <div class="card border-0 shadow-sm p-3">
        <table class="table small align-middle">
          ...
          <tbody>
            <?php foreach ($dsPhongAdmin as $p): ?>
              <tr>
                <td><?= $p['MaPhong'] ?></td>
                <td><strong><?= $p['SoPhong'] ?></strong></td>
                <td><?= $p['TenKS'] ?></td>
                <td><?= $p['TenLoai'] ?></td>
                <td><?= getStatusBadgePhong($p['TrangThai']) ?></td>
                <td>
                  <form method="POST" class="d-inline border p-1 rounded bg-white">
                    <input type="hidden" name="action" value="suaPhong">
                    <input type="hidden" name="maPhong" value="<?= $p['MaPhong'] ?>">
                    <select name="trangThai" onchange="this.form.submit()" class="form-select form-select-sm d-inline-block w-auto py-0">
                      <option value="Available" <?= $p['TrangThai'] == 'Available' ? 'selected' : '' ?>>Trống</option>
                      <option value="Reserved" <?= $p['TrangThai'] == 'Reserved' ? 'selected' : '' ?>>Đã đặt</option>
                      <option value="Occupied" <?= $p['TrangThai'] == 'Occupied' ? 'selected' : '' ?>>Đang ở</option>
                      <option value="Cleaning" <?= $p['TrangThai'] == 'Cleaning' ? 'selected' : '' ?>>Dọn dẹp</option>
                    </select>
                  </form>
                  ...
                  <form method="POST" class="d-inline" onsubmit="return confirm('Xóa phòng này?')">
                    ...
                    <input type="hidden" name="maPhong" value="<?= $p['MaPhong'] ?>">
                    <button class="btn btn-sm btn-danger py-0">🗑</button>
                  </form>
                </td>
              </tr>
            <?php endforeach; ?>
    ...
    <?php elseif ($trang === 'accounts'): ?>
      <div class="d-flex justify-content-between mb-3">
        <h3 class="font-playfair mb-0">Tài khoản</h3><button class="btn btn-gold btn-sm" data-bs-toggle="modal" data-bs-target="#mTK">+ Thêm user</button>
      </div>
      <div class="card border-0 shadow-sm p-3">
        <table class="table small align-middle">
          <thead>
          ...
          <tbody><?php foreach ($dsTaiKhoanAdmin as $u): ?><tr>
                <td><?= $u['MaTK'] ?></td>
                <td><?= h($u['HoTen']) ?></td>
                <td><?= h($u['Email']) ?></td>
                <td><?= h($u['SDT']) ?></td>
                <td><span class="badge bg-secondary"><?= $u['VaiTro'] ?></span></td>
                <td><?php if ($u['MaTK'] != $_SESSION['MaTK']): ?><form method="POST"><input type="hidden" name="action" value="xoaTaiKhoan"><input type="hidden" name="maTK" value="<?= $u['MaTK'] ?>"><button class="btn btn-sm btn-danger">🗑</button></form><?php endif; ?></td>
