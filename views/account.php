...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && ($_POST['action'] ?? '') === 'capNhatThongTin') {
  ...
  if (!$hoTenMoi || !$sdtMoi) {
    ...
    if ($matKhauMoi !== '') {
      // ĐÃ GỠ BĂM MẬT KHẨU: Lưu trực tiếp mật khẩu thô vào CSDL
      $stmtU = $pdo->prepare("UPDATE tai_khoan SET HoTen = :ht, SDT = :sdt, MatKhau = :mk WHERE MaTK = :id");
      $stmtU->execute([':ht' => $hoTenMoi, ':sdt' => $sdtMoi, ':mk' => $matKhauMoi, ':id' => $maTK]);
    } else {
...
$thongTinTK = $pdo->prepare("SELECT * FROM tai_khoan WHERE MaTK = ?");
$thongTinTK->execute([$maTK]);
$thongTinTK = $thongTinTK->fetch();
...
$paramsB = [':id' => $maTK];
if ($locTrangThai !== '') {
  $sqlB .= " AND ddp.TrangThaiDon = :tt";
  $paramsB[':tt'] = $locTrangThai;
}
$sqlB .= " ORDER BY ddp.NgayTao DESC";
$stmtB = $pdo->prepare($sqlB);
$stmtB->execute($paramsB);
$dsDonDat = $stmtB->fetchAll();
...
$thongKe->execute([$maTK]);
$thongKe = $thongKe->fetch();

$pageTitle = 'Luxury Hotel – Tài khoản';
$activePage = 'account';
    ...
    <div class="col-12 col-lg-3">
      <div class="profile-sidebar border rounded p-4 text-center">
        ...
        <div class="border-top pt-3 text-start small">
          <p class="mb-1">📱 <?= h($thongTinTK['SDT']) ?></p>
          <p class="mb-3">✉️ <?= h($thongTinTK['Email']) ?></p>
    ...
    <div class="col-12 col-lg-9">
      ...
      <div class="tab-content">
        <div class="tab-pane fade show active" id="tab-bookings">
          ...
          <?php endforeach;
          if (empty($dsDonDat)) echo '<p class="text-muted small">Chưa có đơn hàng nào.</p>'; ?>
        </div>
